...
`Flink`默认支持`Avro`，`Kryo`两种序列化框架，`Kryo`性能较`Avro`好些，但无供跨语言的解决方案且序列化结果的自解释性较`Avro`差；经过功能性能等多方面的调研，计划`Task`间数据传递用`Kryo`，提高数据序列化性能，状态的`checkpoint`，`savepoint`用`Avro`，让状态有更好的兼容&可迁移性；此功能从`Flink 1.7+`开始支持具体关注doc:
